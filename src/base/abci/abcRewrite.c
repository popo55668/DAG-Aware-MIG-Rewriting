...
{
  Rwr_Man_t * p = Rwr_ManStart(0);
    int i, k = 0;
    for( i = 0; i < 65536; i++ )
        if( p->pMap[i] == 0x0000 ){
            k++;
            printf("canon truth = %d ", p->puCanons[i] );
            printf("truth = %d\n", i );
        }
    printf("%d\n", k );
    /*
  ...
  int i;*/
  /*
  Vec_VecForEachLevel( pRwr->vClasses, pTemp, i ) {
      printf("class %d, size = %d ", i, Vec_PtrSize(pTemp) );
      if( Vec_PtrSize(pTemp) ) {
          printf("truth = %d", ((Rwr_Node_t*)Vec_PtrEntry(pTemp,0))->uTruth );
      }
      printf("\n");
  }*/
  //Vec_Ptr_t * pVec;
